...
def generate_examples(windows, skip_chain_left, skip_chain_right, nb_append=None, nb_skip=0, verbose=True):
        ...
        if skipped < nb_skip:
            ...
            for word_idx in range(len(window.tokens)):
                fvl = window.get_feature_values_list(word_idx,
                                                     skip_chain_left, skip_chain_right)
...
class Article(object):
    ...
    def get_label_counts(self, add_no_ne_label=False):
        ...
        if add_no_ne_label:
            ...
            counts = Counter([token.label for token in self.tokens \
                                          if token.label != self.no_ne_label])
...
class Token(object):
    ...
    def __init__(self, original, no_ne_label = 'O', 
                remove_bio_encoding = True, labels = ['PARK', 'CHAR', 'ATTR', 'REST', 'ENTE']):
